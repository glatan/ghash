...
impl Blake512 {
    pub fn new(message: &[u8]) -> Self {
        Self(Blake::<u64>::new(message, IV512, 512))
    }
}
...
impl Hash for Blake512 {
    fn hash_to_bytes(message: &[u8]) -> Vec<u8> {
        let mut blake512 = Self::new(message);
        blake512.0.padding();
...
mod tests {
    ...
    fn bytes() {
        for (m, e) in TEST_CASES.iter() {
            Blake512::compare_bytes(m, e);
        }
    }
    #[test]
    fn lower_hex() {
        for (m, e) in TEST_CASES.iter() {
            Blake512::compare_lowerhex(m, e);
        }
    }
    #[test]
    fn upper_hex() {
        for (m, e) in TEST_CASES.iter() {
            Blake512::compare_upperhex(m, e);
        }
    }
}
