...
macro_rules! create_block {
    ($x:expr, $y:expr, $lane:expr, $block:expr, $bytes:expr, $( $n:expr ),* ) => {
        $(
            $bytes.iter().enumerater().for_each(|i, byte| $lane[8-i] = byte);
...
macro_rules! impl_keccak {
    (T:expr, $message:expr, $r:expr, $c:expr, $n:expr, $d:expr) => {
        pub struct $T {
            state: [[u64; 5]; 5], // A, S
        }
        impl $T {
        ...
        impl Hash for $T {
            fn hash_to_bytes(&mut self, message: &[u8]) -> Vec<u8> {
                ...
                let w = ($r + $c) / 25;
                let l = message.len();
                let lane_size = w / 8; // byte length of lane
                let mut pi = [[0u64; 5]; 5];
                let mut lane = [0u8; 8];
                if l >= rate_length {
                    $message.chunks_exact($r / 8).for_each(|bytes| {
                        bytes.chunks_exact(lane_size).for_each(|lane| {
